...
## Pada saat ingin mengganti warna gradien barplot pada `ggplot2`, sebagai pengganti `scale_fill_gradient()` yang hanya terdiri dari 2 transisi warna, apakah ada fungsi yang memungkinkan kita untuk menggunakan 3 gradasi warna (ada warna tengah)?
...
ggplot(data = top10_channel, mapping = aes(y = reorder(Var1, Freq), x = Freq)) + 
  geom_col(aes(fill = Freq)) + 
  scale_fill_gradient2(high = "black", low = "grey", mid = "red", midpoint=65) +
  geom_text(aes(label = Freq), nudge_x = -5, color = "white")+
  labs(title = "Top 10 Channels with Most Trending Videos",
       y = "Channel Name",
       x = "Number of Trending Videos")
