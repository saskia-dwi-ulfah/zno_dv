...
- `trending_date`: tanggal video tersebut trending (dalam format YYYY-DD-MM).
- `title`: judul video.
...
❓ Dari summary di atas, kolom apa yang bisa di-exclude? ...
...
as.Date(head(vids$publish_time), format = "%Y-%m-%dT%H:%M:%SZ")
```
...
```
❓ Kolom mana yang bisa kita gunakan untuk menjawab business question di atas? 
...
* Menambahkan garis regresi  yang menunjukkan hubungan .
...
# garis regresi
abline(lm(vids_hpt$comments_views ~ vids_hpt$likes_views))
...
* `color`: memberikan warna pada garis tepi.
...
* Di dalam `aes()`: memberikan warna yang berbeda (berdasarkan data).
...
2️⃣ **Step 2*: switch kolom sumbu x dan y pada parameter `aes()`
